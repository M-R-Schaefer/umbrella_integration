...
try:
    ...
    SCIPY = False
    print(
        "An error occurred while importing scipy.integrate, integration with Simpson's method will be disabled: {0}".format(
            e
        )
    )

try:
    from umbrella_integration.helpers.plot_data import (add_axis_to_figure,
                                                        create_figure,
                                                        finalise_figure,
                                                        generate_histogram,
                                                        plot_with_kde,
    ...
    CAN_PLOT = False
    print(
        "An error occurred while importing helpers.plot_data, plotting will be disabled: {0}".format(
            e
        )
    )

try:
    from integration_errors.calculate_error import \
        trapz_integrate_with_uncertainty
...
def run_umbrella_integration(
    ...
    if integration_method == "Simpsons" and not SCIPY:
        logging.error(
            "Cannot integration with Simpson's method due to import error. Please install scipy for this feature. Will continue with Trapezoidal method."
        )
    ...
    if position_histogram_plot:
        if CAN_PLOT:
            ...
            logging.error(
                "Cannot plot histogram due to import error. Please check that you have matplotlib installed."
            )
    ...
    bin_centers = get_bins(
        sorted(input_data.keys()), bin_width, number_bins, minimum_maximum_value
    )
    logging.info(
        "N evaluation points: {0}; evaluation point range: {1}-{2}".format(
            len(bin_centers), bin_centers[0], bin_centers[-1]
        )
    )
    ...
    if plot_derivatives:
        if CAN_PLOT:
            simple_plot(
                ...
                np.sqrt(bin_var),
                r"$\xi$",
                r"$dA/d\xi\ (\mathrm{kJ} \, \mathrm{mol}^{-1})$",
                fig_name=plot_derivatives,
                show_auc=False,
            )
        else:
            logging.error(
                "Cannot plot derivatives due to import error. Please check that you have matplotlib installed."
            )
    if derivatives_file:
        write_derivatives_to_file(
            bin_centers, bin_derivatives, bin_var, derivatives_file
        )

    # integrate derivatives to calculate final PMF, incl. propagation of errors and plot data
    reaction_coordinate_positions, integral = integrate_derivatives(
        bin_centers, bin_derivatives, integration_method
    )
    mu_sigma_windows = np.mean(
        [window_data["sigma_xi_b_i"] for window_data in input_data.values()]
    )
    integral_point_var = integral_point_variance(
        bin_centers,
        bin_derivatives,
        bin_var,
        mu_sigma_windows,
    ...
    if plot_pmf:
        if CAN_PLOT:
            simple_plot(
                reaction_coordinate_positions,
                integral,
                np.sqrt(integral_point_var),
                r"$\xi$",
                r"$A\ (\mathrm{kJ} \, \mathrm{mol}^{-1})$",
                fig_name=plot_pmf,
            )
        else:
            logging.error(
                "Cannot plot PMF due to import error. Please check that you have matplotlib installed."
            )
    ...
    if output_pmf_file:
        outfile_pmf(
            reaction_coordinate_positions,
            integral,
            np.sqrt(np.abs(integral_point_var)),
            output_pmf_file,
        )
    ...
    if integration_method == "Simpsons":
        xs, integral = zip(
            *[
                (
                    bin_centers[i + 1],
                    simps(bin_derivatives[: i + 2], bin_centers[: i + 2]),
                )
                for i in range(len(bin_derivatives) - 2)
            ]
        )
        ...
        xs, integral = zip(
            *[
                (
                    np.mean([bin_centers[i], bin_centers[i + 1]]),
                    np.trapz(bin_derivatives[: i + 1], bin_centers[: i + 1]),
                )
                for i in range(len(bin_derivatives) - 1)
            ]
        )
    else:
        raise Exception(
            "Unrecognised integration method: {0}, please use: {1}".format(
                integration_method, " or ".join(INTEGRATION_METHODS)
            )
        )
    return xs, integral
...
def get_window_functions(input_data, temperature):
    ...
    def probability_b_i(sigma_xi_b_i, mu_xi_b_i, xi):
        return (1.0 / (sigma_xi_b_i * np.sqrt(2.0 * np.pi))) * np.exp(
            -0.5 * (((xi - mu_xi_b_i) / sigma_xi_b_i) ** 2)
        )

    # calculate variance of derivative, given in Eq. 5 in Kaestner and Thiel 2006
    def var_dA_b_u_over_d_xi(
        var_mu_segmented, var_var_segmented, sigma_xi_b_i, mu_xi_b_i, xi
    ):
        return (1.0 / (BETA**2 * sigma_xi_b_i**4)) * (
            var_mu_segmented
            + ((xi - mu_xi_b_i) ** 2 / sigma_xi_b_i**4) * var_var_segmented
        )
    ...
    for eq_pos, umbrella_data in sorted(input_data.items()):
        pmf_window_functions[eq_pos] = {
            "p_b_i": partial(
                probability_b_i,
                umbrella_data["sigma_xi_b_i"],
                umbrella_data["mu_xi_b_i"],
            ),  # set window parameters for Eq. 5 Kaestner and Thiel 2005
            "dA/dxi": partial(
                dA_b_u_over_d_xi,
                eq_pos,
                umbrella_data["sigma_xi_b_i"],
                umbrella_data["mu_xi_b_i"],
                umbrella_data["k"],
            ),  # set window parameters for Eq. 6 Kaestner and Thiel 2005
            "var(dA/dxi)": partial(
                var_dA_b_u_over_d_xi,
                umbrella_data["var_mu_segmented"],
                umbrella_data["var_var_segmented"],
                umbrella_data["sigma_xi_b_i"],
                umbrella_data["mu_xi_b_i"],
            ),  # set window parameters for Eq. 5 in Kaestner and Thiel 2006
        }
...
def calculate_bin_derivatives(window_functions, bin_centers, calculate_var=True):
    ...
    def dA_dxi_bin(xi_bin, p_i_list, dA_dxi_list):
        return np.sum(
            [p_i(xi_bin) * dA_dxi(xi_bin) for p_i, dA_dxi in zip(p_i_list, dA_dxi_list)]
        )
    ...
    def var_dA_dxi_bin(xi_bin, p_i_list, var_dA_dxi_list):
        return np.sum(
            [
                p_i(xi_bin) ** 2 * var_dA_dxi(xi_bin)
                for p_i, var_dA_dxi in zip(p_i_list, var_dA_dxi_list)
            ]
        )
    ...
    p_b_i_list, dA_dxi_list, var_dA_dxi_list, N_i_list = zip(
        *[
            (funcs["p_b_i"], funcs["dA/dxi"], funcs["var(dA/dxi)"], funcs["N"])
            for _, funcs in sorted(window_functions.items())
        ]
    )
    ...
    for i, xi_bin in enumerate(bin_centers):
        ...
        assert total_norm_factor != 0, (
            "Nomalisation factor is equal to zero, this will produce NaN derivatives. "
        ...
        p_i_list = [
            partial(probability_i, N_i, p_b_i, total_norm_factor)
            for p_b_i, N_i in zip(p_b_i_list, N_i_list)
        ]
...
def calculate_bin_derivatives_faster(input_data, bin_centers, temperature):
    ...
    window_means, window_sigmas, force_constants, Ns = zip(
        *[
            (
                np.mean(window_data["com"]),
                np.std(window_data["com"]),
                window_data["k"],
                len(window_data["com"]),
            )
    ...
    for i, xi_bin in enumerate(bin_centers):
        total_weight = 0
        for xi_i, mu_xi_b_i, sigma_xi_b_i, k, N in zip(
            window_eq_xi, window_means, window_sigmas, force_constants, Ns
        ):
            xi_bin_minus_mu_xi_b_i = xi_bin - mu_xi_b_i
            var_xi_b_i = sigma_xi_b_i**2
            weight = (
                N
                * np.exp(-0.5 * (xi_bin_minus_mu_xi_b_i**2 / var_xi_b_i))
                / (sigma_xi_b_i * sqrt_2_pi)
            )
            da_dxi[i] += weight * (
                xi_bin_minus_mu_xi_b_i / (var_xi_b_i * BETA) - k * (xi_bin - xi_i)
            )
            total_weight += weight
...
def get_bins(
    ...
    if bin_width is not None and number_bins is not None:
        raise Exception(
            "Only one of the following arguments can be provided: bin_width, number_bins"
        )
    first_bin_center = (
        window_centers[0] if not minimum_maximum_value else minimum_maximum_value[0]
    )
    last_bin_center = (
        window_centers[-1] if not minimum_maximum_value else minimum_maximum_value[1]
    )
...
def integral_point_variance(
    bin_centers,
    bin_derivatives,
    bin_var,
    mu_sigma_windows,
    integration_method=INTEGRATION_METHODS[0],
    integration_error_reference=INTEGRATION_ERROR_REFERENCE_OPTIONS[0],
    ...
    if (
        ...
        and integration_method == "trapezoidal"
        and CAN_USE_TRAPZ_INTEGRATION_ERRORS
    ):
        if integration_error_reference == "right":
            return [
                trapz_integration_error_analysis(
                    bin_centers[-i - n_remove :],
                    bin_derivatives[-i - n_remove :],
                    bin_var[-i - n_remove :],
                )
        ...
        if integration_error_method == "trapz_analysis":
            raise Exception(
                "To use trapezoidal integration error analysis: (1) select trapezoidal integration method, "
                "(2) ensure the integration_errors (github.com/martinstroet/trapz_errors) module is in your python path."
            )
        else:
            raise Exception(
                "Unknown integration method: {0}".format(integration_error_method)
            )
...
def var_delta_A(var_derivatives, xis, mu_sigma_windows):
    ...
    return mu_var_derivatives * (
        delta_xi * mu_sigma_windows * np.sqrt(2 * np.pi) - 2.0 * mu_sigma_windows**2
    )
...
def calculate_window_statistics(input_data, n_blocks):
    for _, window_data in sorted(input_data.items()):
        ...
        if block_size < MINIMUM_BLOCK_SIZE:
            logging.warning(
                "The number of frames in each block {0} is below the recommended minimum of {1}".format(
        ...
        mu_segmented, var_segmented = get_segmented_window_statistics(
            window_data["com"], block_size
        )
        ...
        window_data["var_mu_segmented"] = np.var(mu_segmented, ddof=1) / float(
            mu_segmented.shape[0]
        )
        # block averaged estimate of the variance of the variance
        window_data["var_var_segmented"] = np.var(var_segmented, ddof=1) / float(
            mu_segmented.shape[0]
        )
...
def maximum_second_derivative_check(derivatives, bin_centers, input_data):
    caveat = (
        "Johannes Kaestner and Walter Thiel 2006 (DOI: 10.1063/1.2206775). NOTE: this implementation uses numerical second derivatives to estimate kappa which can be very sensitive to "
    ...
    nearest_eq_pos = umbrella_eq_pos[
        (np.abs(np.array(umbrella_eq_pos) - bin_center)).argmin()
    ]
    fc = input_data[nearest_eq_pos]["k"]
    kappa = -kappa
    if fc < 6 * kappa:
        logging.info(
            "Lowest second derivative was: {0:.3g} at {1}".format(-kappa, bin_center)
        )
        logging.warning(
            "The harmonic force constant used ({0:.0g} kJ mol-1 distance-2) is less than the recommended cutoff of 6*kappa={1:.0g} kJ mol-1 distance-2: {2}".format(
                fc, 6 * kappa, caveat
            )
        )
    else:
        logging.info(
            "Lowest second derivative was: {0:.3g} at {1} distance (recommended harmonic force constant ~{2:.0g}): {3}".format(
                -kappa, bin_center, 6 * kappa, caveat
            )
        )
...
def window_separation_distance_check(data, temperature):
    ...
    for i in range(len(window_centers) - 1):
        ...
        max_fc = np.max(
            [
                np.max(data[window_centers[i]]["k"]),
                np.max(data[window_centers[i + 1]]["k"]),
            ]
        )
        window_size_cutoff = 3.0 / np.sqrt(BETA * max_fc)
        if window_delta > window_size_cutoff:
            warn_msg = (
                "Distance between umbrellas {0} is greater than the recommended cutoff of 3/sqrt(BETA*K_fc) = {1:.3g}: "
                "Johannes Kaestner and Walter Thiel 2006 (DOI: 10.1063/1.2206775)".format(
                    window_delta, window_size_cutoff
                )
            )
            if warn_msg not in displayed_messages:
                logging.warning(warn_msg)
                logging.debug(
                    "Foce constant was: {0:.3g} kJ mol^-1 distance^-2".format(max_fc)
                )
                logging.debug(
                    "Umbrella equilibrium points used: {0} - {1}".format(
                        window_centers[i], window_centers[i + 1]
                    )
                )
                displayed_messages.append(warn_msg)
        else:
            info_msg = (
                "Distance between umbrellas {0} is within the recommended cutoff of 3/sqrt(BETA*K_fc) = {1:.3g}: "
                "Johannes Kaestner and Walter Thiel 2006 (DOI: 10.1063/1.2206775)".format(
                    window_delta, window_size_cutoff
                )
            )
...
def parse_input_files(input_files):
    ...
    for eq_pos, umbrella_data in data.items():
        ...
        assert (
            np.unique(fc).shape[0] == 1
        ), "All force constants for this window were not equal: {0}".format(eq_pos)
...
def generate_window_distributions(data, position_histogram_plot):
    ...
    fig_name = (
        "{0}.png".format(position_histogram_plot)
        if not "." in position_histogram_plot
        else position_histogram_plot
    )
    finalise_figure(
        fig_distributions,
        ax_distributions,
...
def write_derivatives_to_file(bin_centers, bin_derivatives, bin_vars, output_file):
    derivatives = "\n".join(
        [
            "{0:>10f} {1:>10f} {2:>10f}".format(bin_center, derivative, bin_var)
            for bin_center, derivative, bin_var in zip(
                bin_centers, bin_derivatives, bin_vars
            )
        ]
    )
...
def outfile_pmf(bin_centers, integral, integral_point_std, pmf_output_file):
    with open(pmf_output_file, "w") as outfile:
        outfile.write(
            "\n".join(
                ["# reaction_coordinate  energy(kJ/mol)  error(kJ/mol)"]
                + [
                    "{0:5f} {1:5f} {2:5f}".format(bc, v, e)
                    for bc, v, e in zip(bin_centers, integral, integral_point_std)
                ]
            )
        )
...
def parse_commandline():
    ...
    bin_group.add_argument(
        "-b",
        "--bin_width",
        help="Bin width along the reaction coordinate (independent of umbrellas used in the sampling).",
        action="store",
        type=float,
        required=False,
    )
    bin_group.add_argument(
        "-n",
        "--number_bins",
        help="Number of evaluation points to be used along the reaction coordinate (independent of umbrellas used in the sampling).",
        action="store",
        type=int,
        required=False,
    )
    parser.add_argument(
        "-m",
        "--minimum_maximum_value",
        help="Minimum and maximum reaction coordinate positions to be considered e.g. -m 0.2 5.5",
        action="store",
        type=float,
        required=False,
        nargs=2,
    ...
    parser.add_argument(
        ...
        help="Integration error estimation reference point: ({0})".format(
            "|".join(INTEGRATION_ERROR_REFERENCE_OPTIONS)
        ),
        action="store",
        required=False,
    )
    parser.add_argument(
        ...
        help="Position histogram plot. File name ending will determine image format; default format is png.",
        action="store",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-d",
        "--derivatives_file",
        help="Write derivative at each evalutation position.",
        action="store",
        required=False,
        default=False,
    )
    parser.add_argument(
        "-nb",
        "--n_blocks",
        help="Number of blocks used in error analysis to calculate block averaged error estimate for each window.",
        action="store",
        type=int,
        required=False,
        default=N_BLOCKS_LOWER_LIMIT,
    )
    parser.add_argument(
        "-im",
        "--integration_method",
        help="Integration method used to numerically integration derivatives.",
        action="store",
        required=False,
        default=INTEGRATION_METHODS[0],
    )
    parser.add_argument(
        "-iem",
        "--integration_error_method",
        help="Method used to estimate numerical integration error.",
        action="store",
        required=False,
        default=INTEGRATION_ERROR_METHODS[0],
    )
