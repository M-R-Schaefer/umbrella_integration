...
def generate_ui_input_lines(input_file, eq_pos, fc, transform_from_relative=False):
    ...
    n = len(time)
    return [
        UI_INPUT_LINE_TEMPLATE.format(t, p, eq_p, f)
        for t, p, eq_p, f in zip(time, pos, [eq_pos] * n, [fc] * n)
    ]
...
def generate_ui_input_lines_merged(run_data, transform_from_relative=False):
    ...
    for input_file, eq_pos, fc in sorted(run_data, key=lambda x: (x[2], x[0])):
        ...
        ui_input_lines.extend(
            generate_ui_input_lines(input_file, eq_pos, fc, transform_from_relative)
        )
...
def parse_input_file(input_file, keep_every=1):
    ...
    lines = np.array(
        [
            ...
            if l.strip()[0] not in SKIP_LINE_CHARACTERS
        ],
        dtype=np.float64,
    )
    fh.close()
...
def parse_commandline():
    ...
    return (
        ...
        args.output_file,
        args.force_constant,
        args.reaction_coordinate_position,
        args.transform_from_relative,
    )


def run_umbrella_integration():
    input_file, output_file, fc, eq_pos, transform_from_relative = parse_commandline()
    ui_input_lines = generate_ui_input_lines(
        input_file, eq_pos, fc, transform_from_relative
    )
...
if __name__ == "__main__":
    ...
    if False:
        ...
        ui_input_lines = generate_ui_input_lines_merged(
            run_data, transform_from_relative=False
        )
